...
public class SeleniumMethods extends SelectCustom {
		...
		try {
			...
			if (!desc.equals("")) {
				if (bool) {
					...
					Reporter.report(STATUS.FAIL, "Element " + desc + " is NOT Clickable even after waiting for '"
	...
	public boolean navigateToAndVerifyPageUrl(Object element, String url) {
		Reporter.createNode("Navigating to [" + url + "] page");
		boolean bool = false;
		// if(isElementPresent(obj,"link for '"+desc+"' Page ")){
		click(element);
	...
	public boolean verifyPageUrl(String url, boolean runReport) {
		...
		try {
			...
			if (runReport) {
				if (bool) {
					Reporter.report(STATUS.PASS, "'<b>" + url + "</b>' page is displayed <br/>" + getCurrentUrl());
				} else {
					Reporter.report(STATUS.FAIL, "'<b>" + url + "</b>' page is NOT displayed <br/>" + getCurrentUrl());
	...
	public boolean verifyPageTitle(String title, boolean runReport) {
		...
		try {
			...
			if (runReport) {
				if (bool) {
					Reporter.report(STATUS.PASS, "'<b>" + title + "</b>' page is displayed <br/>" + getCurrentUrl());
				} else {
	 ...
	 * page it will wait for {@link Constant}.wait seconds. Will take Snapshot, Will
		...
		try {
			...
			if (!desc.equals("")) {
				if (bool) {
					...
					Reporter.report(STATUS.FAIL, "All Elements of " + desc + " are NOT removed after waiting for '"
							+ timeOutInSeconds + "' seconds");
				}
			}
		}
